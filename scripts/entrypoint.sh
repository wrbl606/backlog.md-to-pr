...
ticket_len="${#INPUT_BACKLOG_TASK_ID}"
title_max=72
title_summary_max=$((title_max - ticket_len - 2))
if [[ "$title_summary_max" -lt 10 ]]; then
  title_summary_max=10
fi
if [[ "${#summary}" -gt "$title_summary_max" ]]; then
  title_summary="${summary:0:$title_summary_max}..."
else
...
title="$(replace_token "${INPUT_PR_TITLE:-{ticket}: {summary}}" "$INPUT_BACKLOG_TASK_ID" "$title_summary")"
...
if [[ "${INPUT_TAG_TRIGGER:-false}" == "true" && -n "${GITHUB_ACTOR:-}" ]]; then
